...
export function HomePage() {
  ...
  return (
    <div className="grid gap-7 sm:gap-9">
      <section className="landing-hero relative overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950/80 text-white shadow-[0_28px_70px_-48px_rgba(0,0,0,0.9)]">
        ...
        <div className="relative grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="grid content-between gap-8 p-6 sm:p-8 lg:min-h-[30rem] lg:p-10">
            ...
            <div className="landing-reveal landing-reveal-delay grid gap-3 sm:flex sm:flex-wrap">
              <a
                className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-emerald-400 px-5 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-300/30"
                href="#rooms"
              >
                Choose a room
                <ArrowRightIcon className="size-4 transition group-hover:translate-x-0.5" aria-hidden="true" />
              </a>
              <Link
          ...
          <div className="border-t border-white/10 bg-white/[0.03] p-4 sm:p-6 lg:border-l lg:border-t-0 lg:p-8">
            <div className="landing-panel grid h-full content-between gap-5 rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] sm:p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="landing-pulse rounded-full border border-emerald-300/25 bg-emerald-300/10 px-3 py-1 text-xs font-semibold text-emerald-200">
                  Weekdays
                </span>
              </div>
              ...
              <div className="grid gap-3">
                {[
                  ...
                  ['10:00', 'Team review', 'bg-cyan-200'],
                  ['13:00', 'Open block', 'bg-amber-200'],
                  ['15:00', 'Planning', 'bg-emerald-300'],
                ].map(([time, label, color], index) => (
                  <div
                    key={time}
                    className="landing-slot grid grid-cols-[4.5rem_1fr] items-center gap-3"
                  ...
                  >
                    ...
                    <span className="flex min-h-11 items-center gap-3 rounded-full border border-white/10 bg-white/[0.06] px-3 text-sm text-zinc-200">
                      <span className={`landing-dot size-2 rounded-full ${color}`} />
                      {label}
                    </span>
                  </div>
                ))}
              ...
              <div className="landing-marquee overflow-hidden rounded-full border border-white/10 bg-zinc-950/45 py-2 text-xs font-semibold uppercase tracking-wide text-zinc-300">
