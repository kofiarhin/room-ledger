...
function Initials({ name }) {
  ...
  const letters = parts.length >= 2
    ...
    : (parts[0] || '?').slice(0, 2)
  return (
    <span className="grid size-9 shrink-0 place-items-center rounded-full bg-white/[0.08] text-xs font-bold uppercase tracking-wide text-zinc-300">
      {letters.toUpperCase()}
    </span>
  )
}
...
export function AdminBookingTable({ bookings }) {
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] shadow-[0_18px_44px_-34px_rgba(0,0,0,0.85)]">
      <div className="hidden border-b border-white/10 bg-zinc-950/35 px-5 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500 md:grid md:grid-cols-[2fr_1fr_1.1fr_auto] md:items-center md:gap-4">
      ...
      <div className="divide-y divide-white/10">
        {bookings.map((booking) => (
          <article
            key={booking.id}
            className={`border-l-4 transition duration-150 hover:bg-white/[0.05] ${STATUS_BORDER[booking.status] ?? 'border-l-zinc-600'}`}
          >
            <button
              type="button"
              className="grid w-full gap-4 px-4 py-4 text-left transition focus:outline-none focus:ring-2 focus:ring-inset focus:ring-emerald-300/20 active:scale-[0.999] sm:px-5 md:grid-cols-[2fr_1fr_1.1fr_auto] md:items-center md:gap-4"
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-sm font-semibold text-zinc-100">{booking.requesterName}</p>
            ...
            {expandedId === booking.id && (
              <div className="px-4 pb-5 sm:px-5">
                <div className="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/35">
                      ...
                      <div className="grid gap-4">
                        {booking.purpose && (
                          <div>
                            ...
                            <p className="mt-1.5 text-sm leading-6 text-zinc-400">{booking.purpose}</p>
                          </div>
                        )}
